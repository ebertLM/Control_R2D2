classdef TrajectoryPlanningCircle < handle
    ...
    properties (SetAccess = private)
        ...
        dds_c        % aceleración de crucero (m/s^2)
    end
    
    methods
        function this = TrajectoryPlanningCircle(r, d, po, so, centralAngle, ...
        ...
        function [PD, dPD, ddPD] = PosVelAcc(this)
            ...
            for k = 1:nPoints
                ...
                % Posicion, velocidad y aceleracion en 't'
    ...
    methods
        ...
        function [s, ds, dds] = profile(this, t)
            ...
            if t <= tc_
                s   = so_ + 0.5 * dds_c_ * t^2;
                ds  = dds_c_ * t;
                dds = dds_c_;
            elseif t <= (tf_ - tc_)
                s   = so_ + dds_c_ * tc_ * (t - 0.5*tc_);
                ds  = dds_c_ * tc_;
                dds = 0;
            elseif t <= tf_
                dt  = tf_ - t;
                s   = this.sf - 0.5*dds_c_*dt^2;
                ds  = dds_c_ * dt;
                dds = -dds_c_;
            else
                s   = this.sf;
