/*
  Tracking_circle_V5.ino
  -----------------------
  Seguimiento en tiempo real en espacio operacional para un robot de 3-DOF siguiendo una trayectoria circular.

  Entradas:
    • Conteos de encoders de cada articulación (cuadratura, x4)
    • Comando "start" vía Serial para iniciar la trayectoria
  ...
  Características de la trayectoria:
    • Trayectoria circular en el plano Y–Z a X = X0 (genérico)
    • Centro en [X0, Y0, Z0]
    • Radio = R

  Posiciones iniciales (Home) para el robot R2D2:
    • Articulaciones (q1, q2, q3): [0, pi/2, -pi/2] rad
    • Efector final (x1, x2, x3): [210, 0, 300] mm
  ...
  Dependencias:
    ...
    • TrajectoryPlanning – planificador de trayectoria circular
  ...
  NOTA:
    1. Primero ajustar la fuente para que pwm=max genere Va en cada motor
    2. Chequear el valor de CPR de acuerdo al motor
    3. Verificar que pwm>0 genere wm>0
    4. Descripción del Encoder:
       - Fotoeléctrico CHR-GM37-3429 90:1 (PPR=448)
         ticks total (x4) = 448*4*90 = 161280
       - Fotoeléctrico GA25-371 74.8:1 (PPR=360)
         ticks total (x4) = 360*4*74.8 = 107712
*/
...
BLA::Matrix<3,3> Kp_3DOF = { 225, 0,   0,
                             0,   225, 0,
                             0,   0,   225 };   // Ganancia proporcional
BLA::Matrix<3,3> Kd_3DOF = { 30,  0,   0,
                             0,   30,  0,
                             0,   0,   30 };    // Ganancia derivativa                            
...
//=== Definición de parámetros para el círculo (metros) ===
const float so = 0.0f;  // Longitud inicial del perfil del círculo
BLA::Matrix<3, 1> r =  {1, 0, 0}; // Orientación del vector perpendicular al circulo
BLA::Matrix<3, 1> d =  {0.210f, 0, 0.300f}; // Punto de paso de 'r'
BLA::Matrix<3, 1> po =  {0.149f, 0.149f, 0.300f}; // Punto inicial del círculo
float centralAngle = 2*PI; // Angulo central planificado (rad)
...
float tf=0.0f; // Tiempos de navegación total
float t=0.0f, tc=0.0f, td=0.0f;  // Tiempo muestreado, tiempo de crucero  y tiempo total
...
float **Q, **Vc;                   // Buffer Q[3][lengthColumn] para q1,q2,q3
...
String inputString = "",     // Variable para almacenar la cadena recibida 
       nameMatrixQ = "vectorQ", // Nombre del vector a enviar
       nameMatrixVc = "vectorVc",
       message2start = "Starting...", // Mensaje de inicio de telemetría
       message2finished = "Finished..."; // Mensaje de fin de telemetría
...
TrajectoryPlanning circlePlanner(r, d, po, so, centralAngle, 
...
void setup() {
  ...
  circlePlanner.computeParameters();
  tf  = circlePlanner.getTf(); 
  td  = tf + tadd;
...
void loop() {
  ...
  t = k * Ts;
  auto profileNow    = circlePlanner.profile(t);
  auto trajectoryNow = circlePlanner.trajectory(profileNow);
  ...
  if (k <= lengthColumn) 
  {
    // Almacenar los angulos de las juntas en tiempo real
    Q[0][k-1] = M1_ang;
    Q[1][k-1] = M2_ang;
    Q[2][k-1] = M3_ang;

    // Almacenar los voltajes de control en tiempo real
    Vc[0][k-1] = VuC(0);
    Vc[1][k-1] = VuC(1);
    Vc[2][k-1] = VuC(2);
  }

  //--- H. Envío de datos por el puerto Serial al terminar la recopilación ---
  else 
  {
